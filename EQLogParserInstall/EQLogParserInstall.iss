...
[Files]
; Neither speech engine is installed here. Piper's native library, espeak-ng data and voices, and Kokoro's support
...
Source: "{#MyReleaseDir}\EQLogParser.Core.dll"; DestDir: "{app}"; Flags: ignoreversion
...
; Deliberately NOT deleted: anything the speech engines used to live in. Releases through 2.3.61 installed Piper at
; {app}\piper-tts and its voices under {app}\piper-tts\voices; 2.3.62 reads speech only from
; %LOCALAPPDATA%\EQLogParser\<engine>-<version>, so those files are inert -- but users put their own Piper voices in that
